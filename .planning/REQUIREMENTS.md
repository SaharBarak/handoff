...
- [ ] **SESS-08**: Automatic retention policy — session nodes older than 30 days age out unless they contain "key signals" (commit hashes, external API calls, secret-scanner matches that were blocked). Key-signal sessions retained indefinitely. Configurable via `config.yaml sessions.retention_days`.
